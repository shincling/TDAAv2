...
aim_file='/data1/shijing_data/libricss/OV20/overlap_ratio_20.0_sil0.1_1.0_session0_actual20.8/record/utterances/utterance_35.wav'
aim_file='/data1/shijing_data/libricss/OV20/overlap_ratio_20.0_sil0.1_1.0_session0_actual20.8/clean/utterances/utterance_35.wav'
config.MAX_LEN=int(sf.read(aim_file)[0].shape[0]/2)

aim_path = '/data1/shijing_data/libricss/'
    ...
    if config.MODE == 1:
        if config.DATASET == 'WSJ0':  # 开始构建数据集
            ...
            for ___ in range(number_samples_all):
                ...
                wav_mix=aim_file
                wav_mix=sf.read(wav_mix)[0]
                wav_mix = wav_mix[:,0] if len(wav_mix.shape)>1 else wav_mix
                wav_mix= resampy.resample(sf.read(wav_mix)[0][:,0],16000, config.FRAME_RATE, filter='kaiser_best')
