...
from predata_fromList_123_2rounds import prepare_data  # 数据准备的模块
...
parser.add_argument('-restore', default='../TDAAv4_PIT/data/data/log/2020-01-16-23:07:10/TDAAv3_PIT_284001.pt', type=str,
...
# parser.add_argument('-restore', default='data/data/log/2020-03-31-02:15:11/TDAAv3_PIT_8001.pt', type=str,
# parser.add_argument('-restore', default='data/data/log/2020-03-31-10:53:00/TDAAv3_PIT_tmp.pt', type=str,
...
def train(epoch):
    ...
    while True:
        ...
        del loss,sgm_loss,ss_loss,outputs,pred,multi_mask,train_data,padded_source,padded_mixture,src,feas_tgt
        # continue
...
def eval(epoch):
    ...
    while True:
        ...
        if config.infer_classifier=='arc_margin':
            tgt = Variable(torch.from_numpy(np.array([list(range(top_k+2)) for __ in range(config.batch_size)], dtype=np.int))).transpose(0, 1)  # 转换成数字，然后前后加开始和结束符号。
        else:
