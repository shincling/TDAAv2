...
def split_forTrainDevTest(given_list, train_or_test, phase):
    ...
    if phase == 'spks':  # 用来划分说话人是 train/valid 的 还是test的（不重合）
        if train_or_test == 'train' or train_or_test == 'valid':
            ...
            output_list = [name for name in given_list if
                           name[-1] == '0']  # test spks covers 10% which not overlaped with train or valid.

    elif phase == 'wavs':  # 用来划分训练测试说话人里的没一条语音 train还是valid的语音
        if train_or_test == 'train':
            output_list = [wav for wav in given_list if
                           int(wav[-5]) in range(8)]  # test spks covers 10% which not overlaped with train or valid.
        ...
        if config.DATASET == 'CN':  # 开始构建数据集
            ...
            for ___ in range(number_samples_all):  # 每一步合成一条语音
                ...
                for k, spk in enumerate(aim_spk_k):  # 对于每个采样出来的说话人的一条
                    ...
                    aim_wavs = split_forTrainDevTest(wavs_this_spk, train_or_test,
                                                     phase='wavs')  # find the spks with given train/valid/ or test
                ...
                if config.IS_LOG_SPECTRAL:
                    feature_mix = np.log(np.transpose(np.abs(
                        librosa.core.spectrum.stft(wav_mix, config.FRAME_LENGTH, config.FRAME_SHIFT, ))) + np.spacing(
                        1))
                else:
                    feature_mix = np.transpose(
                        np.abs(librosa.core.spectrum.stft(wav_mix, config.FRAME_LENGTH, config.FRAME_SHIFT, )))
                ...
                batch_idx += 1
                print('batch_dix:{}/{},'.format(batch_idx, config.batch_size), end=' ')
                ...
                if batch_idx == config.batch_size:  # 填满了一个batch
                    ...
                    print(( 'spk_list_from_this_gen:', [list(one.keys()) for one in multi_spk_fea_list]))
