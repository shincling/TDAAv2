...
def train(epoch):
    ...
    while True:
        try:
            ...
            outputs, targets, multi_mask = model(src, src_len, tgt, tgt_len,
                                                 dict_spk2idx)  # 这里的outputs就是hidden_outputs，还没有进行最后分类的隐层，可以直接用
            print
            'mask size:', multi_mask.size()
            ...
            print
            'loss for SGM,this batch:', sgm_loss.data[0] / num_total
            ...
            if 1 and len(opt.gpus) > 1:
                if config.MLMSE:
                    ...
                    lera.log_image(u'Var weight',
                                   Var.data.cpu().numpy().reshape(config.speech_fre, config.speech_fre, 1).repeat(3, 2),
                                   clip=(-1, 1))
            ...
            # dis_loss model
            if config.is_dis:
                dis_loss = models.loss.dis_loss(config, topk_max, model_dis, x_input_map_multi, multi_mask, feas_tgt,
                                                func_dis)
                loss = loss + dis_loss
...
def eval(epoch):
    ...
    while True:
        ...
        if eval_data == False:
            print
            'SDR_aver_eval_epoch:', SDR_SUM.mean()
            break  # 如果这个epoch的生成器没有数据了，直接进入下一个epoch
        ...
        if test_or_valid == 'valid':
            ...
            print
            'loss for ss,this batch:', ss_loss.data[0]
            lera.log({
        ...
        if batch_idx <= (500 / config.batch_size):  # only the former batches counts the SDR
            ...
            print
            'Best SDR from {}---->{}'.format(best_SDR, SDR_SUM.mean())
