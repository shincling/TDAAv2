...
def eval_bss(mix_hidden_layer_3d, adjust_layer, mix_speech_classifier, mix_speech_multiEmbedding, att_speech_layer,
    ...
    while True:
        ...
        x_input_map_multi = x_input_map.view(config.batch_size, 1, mix_speech_len, speech_fre).expand(config.batch_size,
                                                                                                      top_k_num,
                                                                                                      mix_speech_len,
                                                                                                      speech_fre)
        ...
        print
        'loss:', loss_multi_speech.data.cpu().numpy()
        ...
        print
        'SDR_aver_now:', SDR_SUM.mean()

    SDR_aver = SDR_SUM.mean()
    print
    'SDR_SUM (len:{}) for epoch eval : '.format(SDR_SUM.shape)
...
def main():
    ...
    print
    'Begin to build the maim model for Multi_Modal Cocktail Problem.'
    ...
    mix_hidden_layer_3d
    print
    mix_speech_classifier
    print
    mix_speech_multiEmbedding
    att_layer = ATTENTION(config.EMBEDDING_SIZE, 'dot')
    att_speech_layer = ATTENTION(config.EMBEDDING_SIZE, 'dot')
    ...
    for epoch_idx in range(config.MAX_EPOCH):
        ...
        if epoch_idx > 0:
            print
            'SDR_SUM (len:{}) for epoch {} : '.format(SDR_SUM.shape, epoch_idx - 1, SDR_SUM.mean())
        SDR_SUM = np.array([])
        ...
        while 1 and True:
            ...
            # loss_multi_speech=loss_multi_speech #todo:以后可以研究下这个和为１的效果对比一下，暂时直接MSE效果已经很不错了。
            print
            'loss 1, losssum : ', loss_multi_speech.data.cpu().numpy(), loss_multi_sum_speech.data.cpu().numpy()
            lrs.send('loss mask:', loss_multi_speech.data.cpu()[0])
            lrs.send('loss sum:', loss_multi_sum_speech.data.cpu()[0])
            loss_multi_speech = loss_multi_speech + 0.5 * loss_multi_sum_speech
            print
            'training multi-abs norm this batch:', torch.abs(y_multi_map - predict_multi_map).norm().data.cpu().numpy()
            print
            'loss:', loss_multi_speech.data.cpu().numpy()
        ...
        if 1 and epoch_idx % 3 == 0:
            eval_bss(mix_hidden_layer_3d, adjust_layer, mix_speech_classifier, mix_speech_multiEmbedding,
                     att_speech_layer,
                     loss_multi_func, dict_spk2idx, dict_idx2spk, num_labels, mix_speech_len, speech_fre)
